    ...
    swalWithBootstrapButtons
      ...
      .then((result) => {
        if (result.isConfirmed) {
          ...
          swalWithBootstrapButtons.fire(
            "Cancelled",
            "Congratulations on continuing your exploration of rental appliances :)",
            "info"
          );
        }
      });
  ...
  return (
    <header className="navbar navbar-expand-lg navbar-light bg-white">
      <nav className={`container ${styles["nav-pad"]}`}>
        ...
        <img alt='rentitues' src='/images/applianceRentalIcon.png' style={{display:'block', height: '80px', width:'80px'}} />
        ...
        <section className="collapse navbar-collapse" id="navbarNav">
          <ul className="navbar-nav ms-auto mb-2 mb-lg-0">
            ...
            <li className={`nav-item ${styles["nav-item"]}`}>
              <NavLink
                to={"/history?search=&by=id&order=desc"}
                className={`nav-link ${styles["nav-link"]}`}
              >
                <p>History</p>
              </NavLink>
                      ...
                      <React.Fragment>
                        <li>
                          <Link
                            ...
                            to={
                              "/appliance_product?search=&type=&location=&by=id&order=desc"
                            }
                          >
                            Appliances Product
                          </Link>
                        ...
                        <li>
                          <Link
                            className="dropdown-item"
                            to={
                              "/history_renter?search=&filter=&by=id&order=desc"
                            }
                          >
                            Historys Order
                          </Link>
