...
const ErrorServer = () => {
  ...
  useEffect(() => {
    dispatch(listAppliancePopularAction(paramsPopulerAppliance)).then((res) => {
      if (res) {
        navigate("/");
      }
    });
        ...
        <div className="container">
          <div className={styles["err-page"]}>
            ...
            <h4 className={styles["err-about-text"]}>
              if you still want to try exploring the appliance rental website you
              can remind me to change my backup REST API :{" "}
              <a
                href="mailto:hazgnxv@gmail.com?subject=Server/Website Appliance Rental is Error 500"
                className={styles["orange-text"]}
              >
