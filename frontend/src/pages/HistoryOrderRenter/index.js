...
const HistoryOrderRenter = () => {
  ...
  const NewUlrHistoryRenter = `/history_renter?search=${search}&filter=${filter}&by=${by}&order=${order}`;
  ...
  const returnHandler = () => {
    returnHistory(token, dataReturn)
      .then((res) => {
        setIsModal(false);
        setIsReturn(false);
        Swal.fire("Success Return", `${res.data.result.msg}`, "success");
        listHistoryRenter(token, ulrHistoryRenter)
          .then((res) => {
            setHistoryRenter(res.data.result.data);
          })
      ...
      .catch(({ ...err }) => {
        Swal.fire("There an Error?", `${err.response.data.err}`, "error");
        console.log(err);
      });
  };
  ...
  const checkDelHandler = (e) => {
    if (e.target.checked === true)
      setCheckDelRenter([...checkDelRenter, e.target.value]);
  ...
  const deleteItemHandler = () => {
    delHistoryRenter(token, checkDelRenter)
      .then((res) => {
        Swal.fire("Process Complete", `${res.data.result.msg}`, "success");
        setIsModal(false);
        listHistoryRenter(token, ulrHistoryRenter)
          .then((res) => {
            setHistoryRenter(res.data.result.data);
  ...
  return (
    <Main>
      <main className={`container ${styles["top-history"]}`}>
        <h3 style={{ textAlign: "left" }}>Control Panel</h3>
        ...
        <section>
          ...
          >
            ...
            <option value={"appliances"}>Name</option>
          </select>
          <button
        ...
        <section style={{ marginTop: 23 }}>
          {historyRenter.length > 0 ? (
            <React.Fragment>
              ...
              <button
                className={styles["btn-select-item"]}
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title id="contained-modal-title-vcenter">
              {isReturn !== false ? "Return Appliance" : "Delete History"}
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            {isReturn !== false ? (
              <h6>
                Are you sure to change the status of the appliance being returned?
              </h6>
            ) : (
              <h6>Are you sure do you want to delete selected item?</h6>
            )}
          </Modal.Body>
          <Modal.Footer>
            {isReturn !== false ? (
              <React.Fragment>
                ...
                <Button className="btn-warning w-25" onClick={returnHandler}>
                  Yes
                </Button>
              </React.Fragment>
            ) : (
              <React.Fragment>
                ...
                <Button
                  className="btn-dark text-warning w-25"
                  onClick={() => {
                    setIsModal(false);
                  }}
                >
