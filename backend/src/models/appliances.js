...
const addNewapplianceModel = (body, files, id) => {
  return new Promise((resolve, reject) => {
    ...
    db.query(checkLocation, [locations_id], (err, result) => {
      ...
      if (name === "")
        return resolve({
          status: 400,
          result: { err: "Please Input appliance Name" },
        });
      if (description === "")
        return resolve({
          status: 400,
      ...
      if (types_id === "")
        return resolve({
          status: 400,
          result: { err: "Please Input Type appliance" },
        });
      ...
      db.query(checkType, [types_id], (err, result) => {
        ...
        const sqlQuery = `INSERT INTO appliances SET ?`;
        db.query(sqlQuery, body, (err, result) => {
          ...
          const idappliance = result.insertId;
          let values = "VALUES";
          const imgArr = [];
          ...
          files.forEach((data, idx) => {
            ...
            imgArr.push(data.filename, idappliance);
          });
          const imgQuery = `INSERT INTO appliances_img (user_id, images, appliance_id) ${values}`;
          ...
          db.query(imgQuery, imgArr, (err, result) => {
            ...
            result = { msg: `Success Add new appliance with name ${body.name}` };
...
const listapplianceModels = (query) => {
  return new Promise((resolve, reject) => {
    let sqlQuery = `SELECT v.id, v.name AS "appliance", l.name,
        t.name AS "types", v.stock,
        (SELECT CAST(AVG(rating) AS DECIMAL(10,1)) FROM historys where appliances_id = v.id) AS rating
        , v.price, u.name AS "owner",
        (SELECT images FROM appliances_img WHERE appliance_id = v.id LIMIT 1) AS image
        FROM appliances v
    ...
    let orderBy = "";
    if (query.by && query.by.toLowerCase() == "appliances") orderBy = "v.name";
    ...
    let countQuery = ` SELECT COUNT(*) AS "count" FROM appliances v
        JOIN types t ON v.types_id = t.id
        JOIN users u ON v.user_id = u.id
        JOIN locations l ON v.locations_id = l.id
        `;
    ...
    db.query(countQuery, (err, result) => {
      ...
      let linkResult = "";
      let links = `${process.env.URL_HOST}/appliances?`;
...
const listapplianceByRenterIdModel = (idRenter, query) => {
  return new Promise((resolve, reject) => {
    let sqlQuery = `SELECT v.id, v.name AS "appliance", l.name AS "location",
        t.name AS "types", v.stock,
        (SELECT CAST(AVG(rating) AS DECIMAL(10,1)) FROM historys where appliances_id = v.id) AS rating
        , v.price
        FROM appliances v
...
const applianceDetailModel = (id) => {
  return new Promise((resolve, reject) => {
    const imgQuery = `SELECT images from appliances_img WHERE appliance_id = ?`;
    db.query(imgQuery, [id], (err, result) => {
      ...
      const sqlQuery = `SELECT v.id, v.name AS "appliance", v.description, l.name AS "location", locations_id,
            (SELECT CAST(AVG(rating) AS DECIMAL(10,1)) FROM historys where appliances_id = v.id) AS rating,
...
const updateappliances = (body, id, files, params) => {
  return new Promise((resolve, reject) => {
    const { locations_id, types_id, price } = body;
    const numberPatern = /^[0-9]*$/;

    const checkIdRenter = `SELECT * FROM appliances WHERE id = ? AND user_id = ${id}`;
    db.query(checkIdRenter, [params.id], (err, result) => {
      ...
      if (result.length === 0)
        return resolve({
          status: 400,
          result: { err: `You are not the owner of this appliance` },
        });
      if (price === "")
        return resolve({ status: 400, result: { err: "Please Input Price" } });
      if (!numberPatern.test(price))
      ...
      db.query(checkLocation, [locations_id], (err, result) => {
        ...
        db.query(checkType, [types_id], (err, result) => {
          ...
          const checkImage = "SELECT * FROM appliances_img where appliance_id = ?";
          db.query(checkImage, [params.id], (err, result) => {
            ...
            if (result.length > 0) {
              imgArr.forEach((data) => {
                fs.unlink(`public/img/appliances/${data.images}`, (err) => {
                  if (err) {
                    console.log(err);
                  }
                });
            ...
            const sqlQuery = `UPDATE appliances SET ? WHERE id = ? AND user_id = ${id}`;
            db.query(sqlQuery, [body, params.id], (err, result) => {
              ...
              const deleteFiles = `DELETE FROM appliances_img WHERE appliance_id = ? LIMIT ?`;
              ...
              const imgQuery = `INSERT INTO appliances_img (images, appliance_id) ${values}`;
              db.query(imgQuery, imgArr, (err, result) => {
                if (err) {
                  if (files.length !== 0) {
                    deleteImages(files);
...
const delapplianceById = (idappliance, id) => {
  return new Promise((resolve, reject) => {
    const checkId = `SELECT * FROM appliances WHERE id = ${idappliance} AND user_id = ${id}`;
    db.query(checkId, (err, result) => {
      if (err) return reject({ status: 500, err });
      if (result.length === 0)
        return reject({
          status: 401,
          err: "You are not the owner of this appliance",
        });

      const deleteImgQuery = `DELETE FROM appliances_img WHERE appliance_id = ${idappliance}`;
      db.query(deleteImgQuery, (err) => {
        if (err) return reject({ status: 500, err });

        const deleteHistoryQuery = `DELETE FROM historys WHERE appliances_id = ${idappliance}`;
        db.query(deleteHistoryQuery, (err) => {
          if (err) return reject({ status: 500, err });

          const sqlQuery = `DELETE FROM appliances WHERE id = ${idappliance} AND user_id = ${id}`;
          db.query(sqlQuery, (err, result) => {
            if (err) return reject({ status: 500, err });

            result = { msg: "success delete appliance" };
            resolve({ status: 200, result });
          });
        });
      });
...
const deleteImages = (files) => {
  files.forEach((element) => {
    fs.unlink(`public/img/appliances/${element}`, (err) => {
      if (err) {
        console.log(err);
      }
    });
...
module.exports = {
  addNewapplianceModel,
  listapplianceModels,
  listapplianceByRenterIdModel,
