...
const postNewHistory = (body, id, params) => {
  return new Promise((resolve, reject) => {
    ...
    db.query(applianceQuery, [params.id], (err, result) => {
      if (err) return reject({ status: 500, err });
      if (result.length === 0)
        return resolve({ status: 400, result: { err: "appliance not found" } });
      if (quantity === "")
        return resolve({
          status: 400,
      ...
      if (id === user_id)
        return resolve({
          status: 400,
          result: { err: "You are the owner of this appliance" },
        });

      body = {
        ...body,
        appliances_id: params.id,
        users_id: id,
      ...
      const updateStock = `Update appliances SET stock = ? WHERE id = ?`;
...
const getHistory = (id, query) => {
  return new Promise((resolve, reject) => {
    let sqlQuery = `SELECT h.id, v.name, t.name AS "type", h.payment, h.quantity, s.name AS "status",
        ...
        (SELECT create_at FROM historys WHERE appliances_id = v.id LIMIT 1) AS "renter_time",
        h.rating, h.testimony
        FROM historys h
        JOIN appliances v ON h.appliances_id = v.id
...
const putHistoryByIdModel = (body, historyID, userId) => {
  return new Promise((resolve, reject) => {
    ...
    db.query(checkStatus, (err, result) => {
      ...
      const { quantity, appliances_id, status_id } = result[0];
      ...
      db.query(sqlQuery, [body], (err) => {
        ...
        const checkStockQuery = `SELECT stock FROM appliances WHERE id = ${appliances_id}`;
        db.query(checkStockQuery, (err, result) => {
          ...
          const updateStockQuery = `UPDATE appliances set ? WHERE id = ${appliances_id}`;
          db.query(updateStockQuery, updateStock, (err, result) => {
            if (err) return reject({ status: 500, err });

            result = { msg: "appliance status has been successfully restored" };
            resolve({ status: 200, result });
          });
        });
      });
  ...
  return new Promise((resolve, reject) => {
    ...
    db.query(checkStatus, (err, result) => {
      ...
      if (status_id === 2)
        return reject({
          status: 400,
          err: `the appliance has not been returned, can't give a rating and comment`,
        });
...
const delHistoryById = (body, userId) => {
  return new Promise((resolve, reject) => {
    ...
    db.query(checkHistorybyuser, idArr, (err, result) => {
      ...
      if (statusErr === true)
        return reject({ status: 400, err: "Have not returned the appliance" });
  ...
  return new Promise((resolve, reject) => {
    ...
    db.query(checkHistorybyuser, idArr, (err, result) => {
      ...
      if (statusErr === true)
        return reject({
          status: 400,
          err: "appliance renter has not been returned",
        });
